...
void THZTensor_(fill)(THZTensor *r_, real value) {
  TH_TENSOR_APPLY(real, r_, THZVector_(fill)(r__data, value, r__size); break;);
}

void THZTensor_(zero)(THZTensor *r_) {
  TH_TENSOR_APPLY(real, r_, THZVector_(fill)(r__data, 0, r__size); break;);
}
...
void THZTensor_(maskedCopy)(THZTensor *tensor, THByteTensor *mask,
                            THZTensor *src) {
...
void THZTensor_(maskedSelect)(THZTensor *tensor, THZTensor *src,
                              THByteTensor *mask) {
...
void THZTensor_(indexSelect)(THZTensor *tensor, THZTensor *src, int dim,
                             THLongTensor *index) {
  long i, numel;
...
void THZTensor_(indexCopy)(THZTensor *tensor, int dim, THLongTensor *index,
                           THZTensor *src) {
  long i, numel;
  THZTensor *tSlice, *sSlice;
  long *index_data;
...
void THZTensor_(indexFill)(THZTensor *tensor, int dim, THLongTensor *index,
                           real val) {
  long i, numel;
  THZTensor *tSlice;
  long *index_data;
...
accreal THZTensor_(dot)(THZTensor *tensor, THZTensor *src) {
  ...
  TH_TENSOR_APPLY2(real, tensor, real, src,
                   long sz = (tensor_size - tensor_i < src_size - src_i
                                  ? tensor_size - tensor_i
                                  : src_size - src_i);
                   sum += THZBlas_(dot)(sz, src_data, src_stride, tensor_data,
                                        tensor_stride);
                   tensor_i += sz; src_i += sz;
...
real THZTensor_(minall)(THZTensor *tensor) {
  ...
  TH_TENSOR_APPLY(real, tensor, if (CABS(*tensor_data) < CABS(theMin)) theMin =
                                    *tensor_data;);
  return theMin;
}
...
real THZTensor_(maxall)(THZTensor *tensor) {
  ...
  TH_TENSOR_APPLY(real, tensor, if (CABS(*tensor_data) > CABS(theMax)) theMax =
                                    *tensor_data;);
  return theMax;
}
...
void THZTensor_(add)(THZTensor *r_, THZTensor *t, real value) {
  ...
  if (THZTensor_(isContiguous)(r_) && THZTensor_(isContiguous)(t) &&
      THZTensor_(nElement)(r_) == THZTensor_(nElement)(t)) {
...
void THZTensor_(cadd)(THZTensor *r_, THZTensor *t, real value, THZTensor *src) {
  THZTensor_(resizeAs)(r_, t);
  if (THZTensor_(isContiguous)(r_) && THZTensor_(isContiguous)(t) &&
      THZTensor_(isContiguous)(src) &&
      THZTensor_(nElement)(r_) == THZTensor_(nElement)(src)) {
    if (r_ == t) {
      THZBlas_(axpy)(THZTensor_(nElement)(t), value, THZTensor_(data)(src), 1,
                     THZTensor_(data)(r_), 1);
    } else {
      real *tp = THZTensor_(data)(t);
...
void THZTensor_(newFloat)(THZTensor *r_, THFloatTensor *t, THFloatTensor *src) {
  THLongStorage *size = THFloatTensor_newSizeOf(t);
  THLongStorage *stride = THFloatTensor_newStrideOf(t);
  THZTensor_(resize)(r_, size, stride);
  if (THZTensor_(isContiguous)(r_) && THFloatTensor_isContiguous(t) &&
      THFloatTensor_isContiguous(src) &&
      THZTensor_(nElement)(r_) == THFloatTensor_nElement(src)) {
    float *tp = THFloatTensor_data(t);
    float *sp = THFloatTensor_data(src);
    real *rp = THZTensor_(data)(r_);
    long sz = THFloatTensor_nElement(t);
    long i;
#pragma omp parallel for if (sz > THZ_OMP_OVERHEAD_THZRESHOLD) private(i)
    for (i = 0; i < sz; i++)
      rp[i] = tp[i] + sp[i] * _Complex_I;
  } else {
    TH_TENSOR_APPLY3(real, r_, float, t, float, src,
                     *r__data = *t_data + *src_data * _Complex_I;);
  }
  free(size);
  free(stride);
}

void THZTensor_(newDouble)(THZTensor *r_, THDoubleTensor *t,
                           THDoubleTensor *src) {
  THLongStorage *size = THDoubleTensor_newSizeOf(t);
  THLongStorage *stride = THDoubleTensor_newStrideOf(t);
  THZTensor_(resize)(r_, size, stride);
  if (THZTensor_(isContiguous)(r_) && THDoubleTensor_isContiguous(t) &&
      THDoubleTensor_isContiguous(src) &&
      THZTensor_(nElement)(r_) == THDoubleTensor_nElement(src)) {
    double *tp = THDoubleTensor_data(t);
    double *sp = THDoubleTensor_data(src);
    real *rp = THZTensor_(data)(r_);
    long sz = THDoubleTensor_nElement(t);
    long i;
#pragma omp parallel for if (sz > THZ_OMP_OVERHEAD_THZRESHOLD) private(i)
    for (i = 0; i < sz; i++)
      rp[i] = tp[i] + sp[i] * _Complex_I;
  } else {
    TH_TENSOR_APPLY3(real, r_, double, t, double, src,
                     *r__data = *t_data + *src_data * _Complex_I;);
  }
  free(size);
  free(stride);
...
void THZTensor_(addcmul)(THZTensor *r_, THZTensor *t, real value,
                         THZTensor *src1, THZTensor *src2) {
  if (r_ != t) {
    THZTensor_(resizeAs)(r_, t);
...
void THZTensor_(addcdiv)(THZTensor *r_, THZTensor *t, real value,
                         THZTensor *src1, THZTensor *src2) {
  if (r_ != t) {
    THZTensor_(resizeAs)(r_, t);
    THZTensor_(copy)(r_, t);
...
void THZTensor_(addmv)(THZTensor *r_, real beta, THZTensor *t, real alpha,
                       THZTensor *mat, THZTensor *vec) {
  ...
  if (mat->stride[0] == 1) {
    THZBlas_(gemv)('n', mat->size[0], mat->size[1], alpha,
                   THZTensor_(data)(mat), mat->stride[1], THZTensor_(data)(vec),
                   vec->stride[0], beta, THZTensor_(data)(r_), r_->stride[0]);
  } else if (mat->stride[1] == 1) {
    THZBlas_(gemv)('t', mat->size[1], mat->size[0], alpha,
                   THZTensor_(data)(mat), mat->stride[0], THZTensor_(data)(vec),
                   vec->stride[0], beta, THZTensor_(data)(r_), r_->stride[0]);
  } else {
    THZTensor *cmat = THZTensor_(newContiguous)(mat);

    THZBlas_(gemv)('t', mat->size[1], mat->size[0], alpha,
                   THZTensor_(data)(cmat), cmat->stride[0],
                   THZTensor_(data)(vec), vec->stride[0], beta,
...
void THZTensor_(addmm)(THZTensor *r_, real beta, THZTensor *t, real alpha,
                       THZTensor *m1, THZTensor *m2) {
   ...
   * m1->size[0], m1->size[1], m2->size[0], m2->size[1]); */
  ...
  THZBlas_(gemm)(
      ...
      m1_->size[(transpose_r == 'n' ? 1 : 0)], alpha, THZTensor_(data)(m1_),
      ...
      beta, THZTensor_(data)(r__), r__->stride[(transpose_r == 'n' ? 1 : 0)]);
...
void THZTensor_(addr)(THZTensor *r_, real beta, THZTensor *t, real alpha,
                      THZTensor *vec1, THZTensor *vec2) {
  ...
  if (r_->stride[0] == 1) {
    THZBlas_(gerc)(vec1->size[0], vec2->size[0], alpha, THZTensor_(data)(vec1),
                   vec1->stride[0], THZTensor_(data)(vec2), vec2->stride[0],
    ...
    THZBlas_(geru)(cvec2->size[0], vec1->size[0], alpha,
                   THZTensor_(data)(cvec2), cvec2->stride[0],
...
void THZTensor_(addru)(THZTensor *r_, real beta, THZTensor *t, real alpha,
                       THZTensor *vec1, THZTensor *vec2) {
  if ((vec1->nDimension != 1) || (vec2->nDimension != 1))
    THError("vector and vector expected");
  ...
  if (r_->stride[0] == 1) {
    THZBlas_(geru)(vec1->size[0], vec2->size[0], alpha, THZTensor_(data)(vec1),
                   vec1->stride[0], THZTensor_(data)(vec2), vec2->stride[0],
                   THZTensor_(data)(r_), r_->stride[1]);
  } else if (r_->stride[0] == 1) {
    THZBlas_(geru)(vec2->size[0], vec1->size[0], alpha, THZTensor_(data)(vec2),
                   vec2->stride[0], THZTensor_(data)(vec1), vec1->stride[0],
                   THZTensor_(data)(r_), r_->stride[0]);
  } else {
...
long THZTensor_(numel)(THZTensor *t) { return THZTensor_(nElement)(t); }

void THZTensor_(max)(THZTensor *values_, THLongTensor *indices_, THZTensor *t,
                     int dimension) {
  THLongStorage *dim;
  long i;

  THArgCheck(dimension >= 0 && dimension < THZTensor_(nDimension)(t), 2,
  ...
  TH_TENSOR_DIM_APPLY3(real, t, real, values_, long, indices_, dimension,
                       long theIndex = 0;
                       real theMax = t_data[0]; for (i = 1; i < t_size; i++) {
...
void THZTensor_(min)(THZTensor *values_, THLongTensor *indices_, THZTensor *t,
                     int dimension) {
  THLongStorage *dim;
  long i;
  ...
  TH_TENSOR_DIM_APPLY3(real, t, real, values_, long, indices_, dimension,
                       long theIndex = 0;
                       real theMin = t_data[0]; for (i = 1; i < t_size; i++) {
                         if (CABS(t_data[i * t_stride]) < CABS(theMin)) {
                           theIndex = i;
                           theMin = t_data[i * t_stride];
                         }
                       } *indices__data = theIndex; *values__data = theMin;);
}
...
void THZTensor_(sum)(THZTensor *r_, THZTensor *t, int dimension) {
  ...
  TH_TENSOR_DIM_APPLY2(real, t, real, r_, dimension, accreal sum = 0; long i;
                       for (i = 0; i < t_size; i++) sum += t_data[i * t_stride];
                       *r__data = (real)sum;);
}
...
void THZTensor_(cross)(THZTensor *r_, THZTensor *a, THZTensor *b,
                       int dimension) {
  int i;
  ...
  THArgCheck(dimension >= 0 && dimension < THZTensor_(nDimension)(a), 3,
             "dimension out of range");
...
void THZTensor_(diag)(THZTensor *r_, THZTensor *t, int k) {
  THArgCheck(THZTensor_(nDimension)(t) == 1 || THZTensor_(nDimension)(t) == 2,
...
/* I cut and pasted (slightly adapted) the quicksort code from
   ...
   Thanks man :)
    ...
    Updated Oct 16 2013: change choice of pivot to avoid worst-case being a
   pre-sorted input - Daniel and Julien
    Updated Oct 24 2013: change pivot comparison to strict inequality to avoid
   worst-case on constant input, see Sedgewick, Algorithms in C, Addison Wesley,
   1990, p. 120 - Julien
*/
#define MAX_LEVELS 300
static void THZTensor_(quicksortascend)(real *arr, long *idx, long elements,
                                        long stride) {
  ...
  while (i >= 0) {
    ...
    if (L < R) {
      P = (L + R) >>
          1; /* Choose pivot as middle element of the current block */
      piv = arr[P * stride];
      pid = idx[P * stride];
      rswap = arr[L * stride];
      swap = idx[L * stride];
...
static void THZTensor_(quicksortdescend)(real *arr, long *idx, long elements,
                                         long stride) {
  long beg[MAX_LEVELS], end[MAX_LEVELS], i = 0, L, R, P, swap, pid;
  real rswap, piv;
...
void THZTensor_(sort)(THZTensor *rt_, THLongTensor *ri_, THZTensor *t,
...
void THZTensor_(cat)(THZTensor *r_, THZTensor *ta, THZTensor *tb,
                     int dimension) {
  THLongStorage *size;
  int i;
...

  {
    ...
    THZTensor_(narrow)(nta, NULL, dimension, 0,
                       (dimension < ta->nDimension ? ta->size[dimension] : 1));
...

  {
    ...
    THZTensor_(narrow)(ntb, NULL, dimension,
                       (dimension < ta->nDimension ? ta->size[dimension] : 1),
                       (dimension < tb->nDimension ? tb->size[dimension] : 1));
...
#define TENSOR_IMPLEMENT_LOGICAL(NAME, OP)                                     \
  ...
  void THZTensor_(NAME##Tensor)(THByteTensor * r_, THZTensor * ta,             \
                                THZTensor * tb) {                              \
...
void THZTensor_(std)(THZTensor *r_, THZTensor *t, int dimension, int flag) {
  ...
  THArgCheck(dimension >= 0 && dimension < THZTensor_(nDimension)(t), 3,
             "invalid dimension");
...
void THZTensor_(norm)(THZTensor *r_, THZTensor *t, real value, int dimension) {
  ...
  if (value == 0) {
    ...
    TH_TENSOR_DIM_APPLY2(real, t, real, r_, dimension, accreal sum = 0; long i;
                         for (i = 0; i < t_size; i++) sum +=
                         pow(CABS(t_data[i * t_stride]), value);
...
void THZTensor_(renorm)(THZTensor *res, THZTensor *src, real value,
                        int dimension, real maxnorm) {
  int i;
  THZTensor *rowR, *rowS;

  THArgCheck(dimension >= 0 && dimension < THZTensor_(nDimension)(src), 3,
             "invalid dimension");
